...
test('validation should fail when sequence is missing', () => {
  results = sequenceValidator(missingSequence);
  expect(results)
    .toEqual(errorResponses.missingSequence);
});

test('validation should fail when sequence is an empty string', () => {
  results = sequenceValidator(emptySequence);
  expect(results)
    .toEqual(errorResponses.missingSequence);
});

test('should fail if a sequence is too short after clean-up', () => {
  results = sequenceValidator(junkSequence);
  expect(results)
    .toEqual(errorResponses.invalidSequence);
});

test('should indicate a valid AA sequence', () => {
  results = sequenceValidator(validAminoAcidSequence);
  expectedResult = {
    ...validResponse,
    likelyType: 'aa',
  };
...
test('should indicate invalid for an AA sequence with invalid characters', () => {
  results = sequenceValidator(invalidAminoAcidSequence);
  expect(results)
    .toEqual(errorResponses.invalidSequence);
});

test('should indicate a valid NA sequence', () => {
  results = sequenceValidator(validNucleicAcidSequence);
  expectedResult = {
    ...validResponse,
    likelyType: 'na',
  };
...
test('should indicate invalid for an NA sequence with invalid characters', () => {
  results = sequenceValidator(invalidNucleicAcidSequence);
  expect(results)
    .toEqual(errorResponses.invalidSequence);
});

test('should indicate AA for an NA sequence with ambiguous codes', () => {
  results = sequenceValidator(validAmbiguousNucleicAcidSequence);
  expectedResult = {
    ...validResponse,
    likelyType: 'aa',
  };
...
test('should remove all FASTA description lines', () => {
  results = sequenceValidator(withFASTA);
  expectedResult = {
    ...validResponse,
    likelyType: 'na',
  };
